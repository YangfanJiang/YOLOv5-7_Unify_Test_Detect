...
class LoadImages_realSize:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            if r != 1:  # always resize down, only resize up if training with augmentation
                interp = cv2.INTER_LINEAR #cv2.INTER_AREA if r < 1 else cv2.INTER_LINEAR
                img0 = cv2.resize(img0, (int(w0 * r), int(h0 * r)), interpolation=interp)

            # img0 = cv2.cvtColor(img0, cv2.COLOR_BGR2RGB)
            # img0 = cv2.copyMakeBorder(img0, int(self.stride*0.5), int(self.stride*0.5), int(self.stride*0.5), int(self.stride*0.5), cv2.BORDER_CONSTANT, value=(128, 128, 128))
            # h, w, c = img0.shape
            assert img0 is not None, 'Image Not Found ' + path
            # print(f'images {self.count}/{self.nf} {path}: ', end='')

        # Padded resize
        # self.img_size =np.ceil(self.img_size / self.stride + 0.5).astype(np.int) * self.stride
        img, ratio, pad = letterbox(img0, [int(self.img_size+self.stride), int(self.img_size+self.stride)], auto=False, scaleup=False)
        # img = letterbox(img0, self.img_size, stride=self.stride)[0]
        ...
        return path, img, img0, h0, w0, self.cap
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if exists:
            ...
            tqdm(None, desc=prefix + d, total=n, initial=n, leave=False)  # display cache results
